...
public class IgDocumentConversionServiceImpl implements ConversionService{
	  ...
	  private static IgRepository igService=
		      (IgRepository) context.getBean("igRepository");
...
//		Metadata
	...
	private void addProfileTextSections(Ig newIg, Profile profile,int position) {
		...
		TextSection infra= new TextSection(profile.getId(), profile.getSectionContents(), Type.PROFILE, position, "Message Infrastructue");
	    TextSection profileComponents= new TextSection(profile.getProfileComponentLibrary().getId(),profile.getProfileComponentLibrary().getSectionContents(), Type.PROFILECOMPONENTREGISTRY,1, "Profile Components") ; 
	    TextSection conformanceProfiles= new TextSection(profile.getMessages().getId(),profile.getMessages().getSectionContents(), Type.CONFORMANCEPROFILEREGISTRY,2, "Conformance Profiles") ;
	    TextSection compositeProfiles= new TextSection(profile.getCompositeProfiles().getId(),profile.getCompositeProfiles().getSectionContents(), Type.COMPOSITEPROFILEREGISTRY,3, "Composite Profiles") ;
	    TextSection segments= new TextSection(profile.getSegmentLibrary().getId(),profile.getSegmentLibrary().getSectionContents(), Type.SEGMENTREGISTRY,4, "Segments And Fields Description") ;
        TextSection datatypes= new TextSection(profile.getDatatypeLibrary().getId(),profile.getDatatypeLibrary().getSectionContents(), Type.DATATYPEREGISTRY,5, "Data Types") ;
        TextSection valueSets= new TextSection(profile.getTableLibrary().getId(),profile.getTableLibrary().getSectionContents(), Type.VALUESETREGISTRY,6, "Value Sets") ;
        ...
        newIg.setProfileComponentRegistry(createProfileComponentRegistry(profile.getProfileComponentLibrary()));
	...
	private ProfileComponentRegistry createProfileComponentRegistry(ProfileComponentLibrary profileComponentLibrary) {
	...
	Link createLink(gov.nist.healthcare.tools.hl7.v2.igamt.lite.domain.Datatype obj, int position ) {
      ...
      l.setDomainInfo(new DomainInfo(obj.getHl7Version(),ConversionUtil.convertScope(obj.getScope())));
      return l;
	}
	...
	private ValueSetRegistry createValueSets(TableLibrary tableLibrary) {
	    ...
	    List<Table> ordred= children.stream().sorted((Table t1 , Table t2)->  t1.getBindingIdentifier().compareTo(t2.getBindingIdentifier())).collect(Collectors.toList());
		for( int i = 0 ; i < ordred.size(); i++) {
	...
	private DatatypeRegistry createDatatypes(DatatypeLibrary datatypeLibrary) {
        ...
        List<Datatype> ordred= children.stream().sorted((Datatype t1 , Datatype t2)->  t1.getLabel().compareTo(t2.getLabel())).collect(Collectors.toList());
        for( int i = 0 ; i < ordred.size(); i++) {
            Link link = this.createLink(ordred.get(i), i+1);
            ret.getChildren().add(link);
	...
	private SegmentRegistry createSegments(SegmentLibrary segmentLibrary) {
      ...
      List<Segment> ordred= children.stream().sorted((Segment t1 , Segment t2)->  t1.getLabel().compareTo(t2.getLabel())).collect(Collectors.toList());
      for( int i = 0 ; i < ordred.size(); i++) {
          Link link = this.createLink(ordred.get(i), i+1);
          ret.getChildren().add(link);
	...
	private TextSection createTextSectionFromSection(Section s, String parentId) {
		...
		if(s.getChildSections() !=null && !s.getChildSections().isEmpty()) {
			Set<gov.nist.hit.hl7.igamt.shared.domain.TextSection> children = new HashSet<gov.nist.hit.hl7.igamt.shared.domain.TextSection>();
