...
public class SegmentServiceImpl implements SegmentService {
	...
	public void applyStructure(Segment segment, List<ChangeItemDomain> cItems) throws Exception {
		...
		if (structureChangeItems.containsKey(ChangeType.DELETE)) {
			for (ChangeItemDomain structDelete : structureChangeItems.get(ChangeType.DELETE)) {
				...
				Optional<Field> element = segment.getChildren().stream()
				...
				if (element.isPresent()) {
					...
					throw new Exception("At path " + structDelete.getLocation() + " cannot remove field to position "
			...
			for (ChangeItemDomain structAdd : structureChangeItems.get(ChangeType.ADD)) {
				...
				if (segment.getChildren().stream().noneMatch(elm -> elm.getPosition() == structAdd.getPosition())) {
					...
					if ((segment.getChildren().size() + 1) == field.getPosition()) {
						...
						throw new Exception("At path " + structAdd.getLocation() + " cannot add field to position "
								+ structAdd.getPosition() + " (Not End Of Segment)");
					}
				} else {
					throw new Exception("At path " + structAdd.getLocation() + " cannot add field to position "
