...
export class LibErrorService {
  ...
  reloadIg(error){
    ...
    this.router.navigateByUrl("/datatype-library/"+this.tocService.getIgId());
    //
  }

  showError(error:any) {
    console.log(error);
    this.messageService.add({severity:'error', summary:error.error.error, detail:error.error.message, id:'LIBERROR'});
  }
