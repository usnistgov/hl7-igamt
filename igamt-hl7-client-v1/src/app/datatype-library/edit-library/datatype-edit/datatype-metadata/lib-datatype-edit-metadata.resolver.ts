...
@Injectable()
export  class LibDatatypeEditMetadataResolver implements Resolve<any> {
  ...
  resolve(route: ActivatedRouteSnapshot, rstate: RouterStateSnapshot): Promise<any> {
    const datatypeId = route.params['datatypeId'];
    return this.datatypeService.getDatatypeMetadata(datatypeId);
  }
