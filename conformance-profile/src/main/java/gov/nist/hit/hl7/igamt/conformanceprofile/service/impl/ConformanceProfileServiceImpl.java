...
//    return result;
	...
	public void applyStructure(ConformanceProfile cp, List<ChangeItemDomain> cItems) throws Exception {
		ObjectMapper mapper = new ObjectMapper();
		mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
		Map<ChangeType, List<ChangeItemDomain>> structureChangeItems = cItems.stream()
		...
		if (structureChangeItems.containsKey(ChangeType.DELETE)) {
			for (ChangeItemDomain structDelete : structureChangeItems.get(ChangeType.DELETE)) {
				...
				if (targetList != null) {
					...
					Optional<SegmentRefOrGroup> element = targetList.stream()
							.filter(elm -> elm.getPosition() == structDelete.getPosition()).findFirst();
					if (element.isPresent()) {

						// Remove Bindings
						this.removeElementBindings(createPath(structDelete.getLocation(), element.get().getId()),
								cp.getBinding(), 0);
						targetList.remove(element.get());
					} else {
						throw new Exception(
								"At path " + structDelete.getLocation() + " cannot remove segment to position "
										+ structDelete.getPosition() + " (Does not Exists)");
					}
				}
			}
		}
		...
		if (structureChangeItems.containsKey(ChangeType.ADD)) {
			List<ChangeItemDomain> addList = structureChangeItems.get(ChangeType.ADD).stream()
					.sorted(Comparator.comparingInt(ChangeItemDomain::getPosition)).collect(Collectors.toList());
				...
				if (targetList != null) {
					...
					if (targetList.stream().noneMatch(elm -> elm.getPosition() == structAdd.getPosition())) {
						...
						if ((targetList.size() + 1) == segmentRef.getPosition()) {
							...
							throw new Exception(
									"At path " + structAdd.getLocation() + " cannot add segment to position "
											+ structAdd.getPosition() + " (Not End Of Message)");
						}
					} else {
						throw new Exception("At path " + structAdd.getLocation() + " cannot add segment to position "
								+ structAdd.getPosition() + " (Already Exists)");
					}
				}
			}
		}
	...
	public void applyChanges(ConformanceProfile cp, List<ChangeItemDomain> cItems, String documentId) throws Exception {
		...
		for (ChangeItemDomain item : cItems) {
			if (item.getPropertyType().equals(PropertyType.NAME)) {
				...
				List<CoConstraintBinding> coconstraints = mapper.readValue(jsonInString,
						new TypeReference<List<CoConstraintBinding>>() {
						});
				cp.setCoConstraintsBindings(coconstraints);
			} else if (item.getPropertyType().equals(PropertyType.PROFILEIDENTIFIER)) {
				String jsonInString = mapper.writeValueAsString(item.getPropertyValue());
				item.setOldPropertyValue(cp.getAuthors());
				List<MessageProfileIdentifier> profileIdentifier = mapper.readValue(jsonInString,
						new TypeReference<List<MessageProfileIdentifier>>() {
						});
