...
public class CoConstraintVerifyServiceImpl implements CoConstraintVerifyService {
	...
	private Datatype checkHeaderExists(Map<String, String> acc, DataElementHeader s, Segment segment){
	...
	private boolean checkHeaderData(Map<String, String> acc, CoConstraintTableHeader header, Datatype datatype){
		boolean errors = false;
		boolean complex = datatype instanceof ComplexDatatype;
		boolean coded = datatype.getLabel().matches("^C(N|W)E$");
		boolean varies = datatype.getLabel().equals("VARIES");
		if(header.getContent().getType() == CellType.Ignore) return false;
		if(header.getContent().isCoded() != coded || header.getContent().isComplex() != complex || header.getContent().isVaries() != varies){
			errors = true;
			acc.put("Header "+header.getContent().getPath(), "Element metadata is invalid or has changed");
		}
	...
	private void checkCells(Map<String, String> acc, Datatype datatype, CoConstraintTableHeader header, List<CoConstraintTableCell> cell){
		List<CoConstraintTableCell> cells = cell.stream().filter(x -> {
			return header.getContent().getType() != x.getType() && header.getContent().getType() != CellType.Ignore;
		})
		.collect(Collectors.toList());
	...
	private Datatype getDatatypeAtPath(Segment segment, String[] path){
		...
		if(field != null){
			...
			return path.length == 1 ? dt : getDatatypeAtPath(dt, Arrays.copyOfRange(path, 1, path.length));
		}
		else {
			return null;
		}
