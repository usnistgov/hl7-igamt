...
export class GroupValueSetComponent implements OnInit {
    ...
    groupDropdownOptions: Array<{ label: string; value: string }> = [];
    ...
    cancel(): void {
        ...
        for (const group of Object.keys(this.groupedData)) {
            ...
            const ids = this.groupedData[group].map((item) => item.id);
    ...
    customize() {
        ...
        this.typeToGroupMap['EXTERNAL'] = 'External';
    }
