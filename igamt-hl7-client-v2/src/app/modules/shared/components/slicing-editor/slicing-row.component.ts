...
import {
  IHL7v2TreeNode,
  IResourceRef
} from "../hl7-v2-tree/hl7-v2-tree.component";
@Component({
...
export class SlicingRowComponent<T extends ISlice> implements OnInit {
  ...
  @Input()
  set sliceParams(event: {
    repo: StoreResourceRepositoryService;
    nodes: IHL7v2TreeNode[];
    sliceRow: ISlicing;
    resources: IDisplayElement[];
    parent: IResource;
    ...
    if (this.sliceRow_ != null) {
      const path: IPath = this.pathService.getPathFromPathId(
        event.parent.id + "-" + this.sliceRow_.path
      );
      ...
      this.elementNamingService
        .getPathInfoFromPath(event.parent, this.repository, path_without_parent)
        .subscribe(pathInfo => {
          this.nameDisplay = this.elementNamingService.getStringNameFromPathInfo(
            pathInfo
          );
        });
      ...
      this.display$.subscribe(x => {
        this.available_ = [
          ...this.resources.filter(elm => x.fixedName === elm.fixedName)
        ];
  ...
  getNodeResource(path: IPath): Observable<IResource> {
    return this.hl7V2TreeService
      .getNodeByPath(this.nodes, path, this.repository)
      .pipe(
        mergeMap(node => {
  ...
  addSlice() {
    if (this.sliceRow_.type === ISlicingMethodType.ASSERTION) {
      const conditionalSlice: IConditionalSlice = {
        comments: "",
        flavorId: null,
        assertion: null
      };
      ...
      const conditionalSlice: IOrderedSlice = {
        comments: "",
        flavorId: null,
        position: this.sliceRow_.slices.length + 1
      };
      this.sliceRow_.slices.push(conditionalSlice);
    }
    this.emitRow(ChangeType.UPDATE);
  ...
  getNodeByPath(path: IPath): Observable<IHL7v2TreeNode> {
    return this.hl7V2TreeService.getNodeByPath(
      this.nodes,
      path,
      this.repository
    );
