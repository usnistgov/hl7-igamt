...
public class CoConstraintSerializationServiceImpl implements CoConstraintSerializationService {
    ...
    public Element SerializeCoConstraintCompact(CoConstraintTable coConstraintsTable) {
        if (coConstraintsTable != null) {
            ...
            Element thead = serializeCoConstraintsTableHeaderCompact(coConstraintsTable);
            tableElement.appendChild(thead);
            Element tbody = serializeCoConstraintsTableContentCompact(coConstraintsTable, generateHeadersList(coConstraintsTable),
        ...
        if (coConstraintsTable != null) {
            ...
            if (coConstraintsTable.getCoConstraints() != null) {
                for (CoConstraint row : coConstraintsTable.getCoConstraints()) {
                    serializeRowVerbose(coConstraintsTable, row, generateHeadersList(coConstraintsTable), tbody);
//					tableContent.appendChild(rowElement);
            ...
            if (coConstraintsTable.getGroups() != null) {
                for (CoConstraintGroupBinding coConstraintGroupBinding : coConstraintsTable.getGroups()) {
                    Element groupElement = serializeGroupVerbose(coConstraintsTable, coConstraintGroupBinding, generateHeadersList(coConstraintsTable), calculateGroupNameColspan(coConstraintsTable));
    ...
    private Element serializeCoConstraintsTableContentCompact(CoConstraintTable coConstraintsTable,
                                                              List<CoConstraintHeader> headersList, int calculateGroupNameColspan) {
        Element tableContent = new Element("tbody");
        if (coConstraintsTable.getCoConstraints() != null) {
            int position = 0;
            for (CoConstraint coConstraint : coConstraintsTable.getCoConstraints()) {
                position += 1;
                Element rowElement = serializeRowCompact(coConstraint, headersList, position);
                tableContent.appendChild(rowElement);
            }
        }
        for (CoConstraintGroupBinding coConstraintGroupBinding : coConstraintsTable.getGroups()) {
            Element groupElement = serializeGroupCompact(coConstraintGroupBinding, headersList, calculateGroupNameColspan);
            tableContent.appendChild(groupElement);
        }
        return tableContent;
    }
...
//				}
    ...
    public Element serializeRowCompact(CoConstraint coConstraintTableRow, List<CoConstraintHeader> headersList, int position) {
        Element tr = new Element("tr");
        Element td = new Element("td");
        if (position == 1) {
            td.appendChild(" 1 (Primary)");
    ...
    public Element serializeGroupCompact(CoConstraintGroupBinding coConstraintGroupBinding, List<CoConstraintHeader> headersList,
                                         int colspanTableWidth) {
        Element groupElement = new Element("tr");
        // groupElement.addAttribute(new Attribute("class", "contentTable"));
        Element groupeTableHeader = serializeGroupHeaderCompact(coConstraintGroupBinding, colspanTableWidth);
        if (coConstraintGroupBinding != null && coConstraintGroupBinding.getType().equals(GroupBindingType.CONTAINED)) {
            Element groupeTableContent = serializeGroupContentCompact(((CoConstraintGroupBindingContained) coConstraintGroupBinding).getCoConstraints(), headersList,
                    colspanTableWidth);
    ...
    public Element serializeGroupContentCompact(List<CoConstraint> coConstraints, List<CoConstraintHeader> headersList, int colspanTableWidth) {
        Element tableContent = new Element("tbody");
        if (coConstraints != null) {
            int position = 0;
            for (CoConstraint coConstraint : coConstraints) {
...
//		}
    ...
    private Element serializeGroupContentVerbose(CoConstraintTable coConstraintTable, List<CoConstraint> coConstraints, List<CoConstraintHeader> headersList,
                                                 int colspanTableWidth) {
        Element tableContent = new Element("tbody");
        if (coConstraints != null) {
            for (CoConstraint coConstraint : coConstraints) {
                serializeRowVerbose(coConstraintTable, coConstraint, headersList, tableContent);
            }
        }
        return tableContent;
    }

    private Element serializeGroupVerbose(CoConstraintTable coConstraintTable, CoConstraintGroupBinding coConstraintGroupBinding, List<CoConstraintHeader> headersList, int colspanTableWidth) {
        ...
        groupElement.appendChild(groupeTableHeader);
        if (coConstraintGroupBinding != null && coConstraintGroupBinding.getType().equals(GroupBindingType.CONTAINED)) {
            Element groupeTableContent = serializeGroupContentVerbose(coConstraintTable, ((CoConstraintGroupBindingContained) coConstraintGroupBinding).getCoConstraints(), headersList,
                    colspanTableWidth);
            groupElement.appendChild(groupeTableContent);
        }
        return groupElement;
    ...
    private void serializeRowVerbose(CoConstraintTable coConstraintsTable, CoConstraint coConstraintTableRow, List<CoConstraintHeader> headersList, Element tableContent) {
