...
//    }
	...
	private void checkingStructureForConformanceProfile(ConformanceProfile conformanceProfile,
			CPVerificationResult result, boolean needDeep) {
		Set<StructureElementBinding> sebs = null;
		if (conformanceProfile.getBinding() != null)
			sebs = conformanceProfile.getBinding().getChildren();
		this.checkingSegmentRefOrGroups(conformanceProfile, conformanceProfile.getChildren(), result, null, null, sebs,
				needDeep);
	}

	private void checkingStructureForConformanceProfileForCompliance(ConformanceProfile conformanceProfile,
			CPComplianceResult result) {
		Set<StructureElementBinding> sebs = null;
		if (conformanceProfile.getBinding() != null)
			sebs = conformanceProfile.getBinding().getChildren();
		this.checkingSegmentRefOrGroupsForCompliance(conformanceProfile, conformanceProfile.getChildren(), result, null,
				null, sebs);
	}

	private void checkingSegmentRefOrGroupsForCompliance(ConformanceProfile conformanceProfile,
			Set<SegmentRefOrGroup> segmentRefOrGroups, CPComplianceResult result, String positionPath, String path,
			Set<StructureElementBinding> sebs) {
		if (segmentRefOrGroups != null) {
			segmentRefOrGroups.forEach(srog -> this.checkingSegmentRefOrGroupForCompliance(conformanceProfile, srog,
					result, positionPath, path, this.findSEB(sebs, srog.getId())));
		}
	}
	...
	private void checkingSegmentRefOrGroups(ConformanceProfile conformanceProfile,
			...
			Set<StructureElementBinding> sebs, boolean needDeep) {
		if (segmentRefOrGroups != null) {
			segmentRefOrGroups.forEach(srog -> this.checkingSegmentRefOrGroup(conformanceProfile, srog, result,
					positionPath, path, this.findSEB(sebs, srog.getId()), needDeep));
		}
	}
	...
	private Set<StructureElementBinding> findSEB(Set<StructureElementBinding> sebs, String id) {
		if (sebs != null) {
			for (StructureElementBinding seb : sebs) {
				if (seb.getElementId().equals(id))
					return seb.getChildren();
			}
		}
		return null;
	}

	private void checkingSegmentRefOrGroupForCompliance(ConformanceProfile conformanceProfile, SegmentRefOrGroup srog,
			CPComplianceResult result, String positionPath, String path, Set<StructureElementBinding> sebs) {
		if (srog instanceof SegmentRef) {
			this.chekcingSegmentRefForCompliance(conformanceProfile, (SegmentRef) srog, result, positionPath, path,
					sebs);
	...
	private void checkingSegmentRefOrGroup(ConformanceProfile conformanceProfile, SegmentRefOrGroup srog,
			CPVerificationResult result, String positionPath, String path, Set<StructureElementBinding> sebs,
			boolean needDeep) {
		if (srog instanceof SegmentRef) {
	...
	private void chekcingSegmentRefForCompliance(ConformanceProfile conformanceProfile, SegmentRef sr,
			CPComplianceResult result, String positionPath, String path, Set<StructureElementBinding> sebs) {
		...
		if (ref != null && ref.getId() != null) {
			...
			if (segment != null) {
				...
				this.childComplianceMap.put(positionPath,
						new ComplianceObject(positionPath, path, usage, min, max, null, null));
				this.checkUsageComplianceError(conformanceProfile, conformanceProfile.getId(),
						conformanceProfile.getType(), new CPMetadata(conformanceProfile), usage, positionPath, path,
						result);
				this.checkCardinalityComplianceError(conformanceProfile, conformanceProfile.getId(),
						conformanceProfile.getType(), new CPMetadata(conformanceProfile), min, max, positionPath, path,
						result);

				if (segment.getChildren() != null) {

					for (Field field : segment.getChildren()) {
						this.checkFieldForConformanceProfileForCompliance(conformanceProfile, segment, field,
								positionPath, path, result);
					}
				}
			}
		}
	...
	private void chekcingSegmentRef(ConformanceProfile conformanceProfile, SegmentRef sr, CPVerificationResult result,
			String positionPath, String path, Set<StructureElementBinding> sebs, boolean needDeep) {
		int position = sr.getPosition();
		Usage usage = sr.getUsage();
		int min = sr.getMin();
		String max = sr.getMax();
		...
		if (ref == null || ref.getId() == null)
			result.getErrors()
					.add(new IgamtObjectError("REF_Missing", conformanceProfile.getId(), conformanceProfile.getType(),
							new CPMetadata(conformanceProfile), "In " + path + ", Ref value is missing",
			...
			if (segment == null)
				result.getErrors().add(new IgamtObjectError("Ref_NotAccessable", conformanceProfile.getId(),
						conformanceProfile.getType(), new CPMetadata(conformanceProfile),
						"In " + path + ", Ref object is not accesable", positionPath + "", "FATAL", "Internal"));
			else {
				if (path == null) {
					path = segment.getName();
				} else {
				...
				this.checkUsageVerificationError(conformanceProfile, conformanceProfile.getId(),
						conformanceProfile.getType(), new CPMetadata(conformanceProfile), usage, positionPath, path,
						result);
				...
				if (needDeep) {
					if (segment.getChildren() != null) {
						...
						for (Field field : segment.getChildren()) {
							this.checkFieldForConformanceProfileForVerification(conformanceProfile, segment, field,
									positionPath, path, result);
						}
					}
				}
	...
	private void checkFieldForConformanceProfileForVerification(ConformanceProfile conformanceProfile, Segment segment,
			Field field, String positionPath, String path, CPVerificationResult result) {
		...
		this.checkUsageVerificationErrorForCP(conformanceProfile, segment.getId(), segment.getType(),
				new DTSegMetadata(segment), usage, positionPath, path, result);
				...
				if (datatype instanceof ComplexDatatype) {
					...
					if (cDT.getComponents() != null && cDT.getComponents().size() > 0) {
						for (Component component : cDT.getComponents()) {
							this.checkComponentForConformanceProfileForVerification(conformanceProfile, cDT, component,
									positionPath, path, result);
						}
					}
				}
	...
	private void checkFieldForConformanceProfileForCompliance(ConformanceProfile conformanceProfile, Segment segment,
			Field field, String positionPath, String path, CPComplianceResult result) {
		int position = field.getPosition();
		Usage usage = field.getUsage();
		...
		this.childComplianceMap.put(positionPath,
				new ComplianceObject(positionPath, path, usage, min, max, minLength, maxLength));
		this.checkUsageComplianceError(conformanceProfile, segment.getId(), segment.getType(),
				new DTSegMetadata(segment), usage, positionPath, path, result);
		this.checkCardinalityComplianceError(conformanceProfile, segment.getId(), segment.getType(),
				new DTSegMetadata(segment), min, max, positionPath, path, result);
		this.checkLengthComplianceError(conformanceProfile, segment.getId(), segment.getType(),
				new DTSegMetadata(segment), minLength, maxLength, positionPath, path, result);
		...
		if (ref != null && ref.getId() != null) {
			...
			if (datatype != null) {
				if (datatype instanceof ComplexDatatype) {
					...
					if (cDT.getComponents() != null && cDT.getComponents().size() > 0) {
						for (Component component : cDT.getComponents()) {
							this.checkComponentForConformanceProfileForCompliance(conformanceProfile, cDT, component,
									positionPath, path, result);
						}
					}
				}
	...
	private void checkLengthComplianceError(ConformanceProfile conformanceProfile, String target, Type targetType,
			Object targetMeta, String minLength, String maxLength, String positionPath, String path,
			CPComplianceResult result) {
		if (maxLength != null && this.isInt(maxLength)) {
			ComplianceObject complianceParentObject = this.parentComplianceMap.get(positionPath);
			if (complianceParentObject != null && complianceParentObject.getMaxLength() != null
					&& this.isInt(complianceParentObject.getMaxLength())) {
				...
				if (parentMaxLengthInt < childMaxLengthInt) {
					result.getErrors().add(new IgamtObjectError("MAXLength_Compliance", target, targetType, targetMeta,
							"In " + conformanceProfile.getLabel() + ", " + path + " assigned MaxLength of "
									+ childMaxLengthInt + " is not compliant with MaxLength of " + parentMaxLengthInt,
		...
		if (minLength != null && this.isInt(minLength)) {
			ComplianceObject complianceParentObject = this.parentComplianceMap.get(positionPath);
			if (complianceParentObject != null && complianceParentObject.getMinLength() != null
					&& this.isInt(complianceParentObject.getMinLength())) {
				...
				if (parentMinLengthInt > childMinLengthInt) {
					result.getErrors().add(new IgamtObjectError("MINLength_Compliance", target, targetType, targetMeta,
							"In " + conformanceProfile.getLabel() + ", " + path + " assigned MinLength of "
									+ childMinLengthInt + " is not compliant with MinLength of " + parentMinLengthInt,
							positionPath + "", "WARNING", "User"));
				}
			}
		}
	...
	private void checkLengthVerificationErorr(String target, Type targetType, Object targetMeta,
			SubStructElement element, String minLength, String maxLength, String confLength, String positionPath,
			String path, VerificationResult result) {
		if (this.isLengthAllowedElement(element)) {

			if (!this.isNullOrNA(confLength) && (!this.isNullOrNA(minLength) || !this.isNullOrNA(maxLength))) {
				result.getErrors()
						.add(new IgamtObjectError("Length_Duplicated", target, targetType, targetMeta,
								"In " + path + ", ConfLength and Length are defined at the same time",
								positionPath + "", "WARNING", "User"));
			}
			if (this.isNullOrNA(confLength) && (this.isNullOrNA(minLength) || this.isNullOrNA(maxLength))) {
				result.getErrors()
						.add(new IgamtObjectError("Length_Missing", target, targetType, targetMeta,
								"In " + path + ", Primitive datatype should have one of Length and ConfLength.",
								positionPath + "", "WARNING", "User"));
			} else {
				if (!this.isNullOrNA(minLength)) {
					if (!this.isInt(minLength)) {
						result.getErrors()
								.add(new IgamtObjectError("MinLength_FORMAT", target, targetType, targetMeta, "In "
										+ path + ", Min Length should be number. The current MinLength is " + minLength,
										positionPath, "WARNING", "User"));
					}
				}
				...
				if (!this.isNullOrNA(maxLength)) {
					if (!this.isIntOrStar(maxLength)) {
						result.getErrors().add(new IgamtObjectError(
								"MaxLength_FORMAT", target, targetType, targetMeta, "In " + path
										+ ", Max Length should be number or '*'. The current MaxLength is " + maxLength,
								positionPath, "WARNING", "User"));
					}
				}
				...
				if (!this.isNullOrNA(confLength)) {
					if (!confLength.contains("#") && !confLength.contains("=")) {
						result.getErrors().add(new IgamtObjectError("ConfLength_FORMAT", target, targetType, targetMeta,
								"In " + path
										+ ", Either # or = can used to define truncation. The current ConfLength is "
										+ confLength,
								positionPath, "WARNING", "User"));
					}
				}
					...
					if (!maxLength.equals("*")) {
						if (this.isInt(minLength) && this.isInt(maxLength)) {
							...
							if (minLengthInt > maxLengthInt) {
								result.getErrors().add(new IgamtObjectError("Length_Range", target, targetType,
										targetMeta,
										"In " + path
												+ ", MIN Length value is bigger than MAX Length. The current MinLength is "
												+ minLength + " and current MaxLength is " + maxLength,
										positionPath, "WARNING", "User"));
							}
						}
					}
			...
			if (!this.isNullOrNA(confLength) || (!this.isNullOrNA(minLength) || !this.isNullOrNA(maxLength))) {
				result.getErrors()
						.add(new IgamtObjectError("Length_NotAllowable", target, targetType, targetMeta,
								path + " does not allow Length or ConfLength, because the datatype is not primitive.",
								positionPath + "", "WARNING", "User"));
			}
		}
	}

	private void checkComponentForConformanceProfileForVerification(ConformanceProfile conformanceProfile,
			Datatype datatype, Component component, String positionPath, String path, CPVerificationResult result) {
		...
		this.checkUsageVerificationErrorForCP(conformanceProfile, datatype.getId(), datatype.getType(),
				new DTSegMetadata(datatype), usage, positionPath, path, result);
				...
				if (childDT instanceof ComplexDatatype) {
					...
					if (cDT.getComponents() != null && cDT.getComponents().size() > 0) {
						for (Component childComponent : cDT.getComponents()) {
							this.checkComponentForConformanceProfileForVerification(conformanceProfile, childDT,
									childComponent, positionPath, path, result);
						}
					}
				}
			}
	...
	private void checkComponentForConformanceProfileForCompliance(ConformanceProfile conformanceProfile, Datatype dt,
			Component component, String positionPath, String path, CPComplianceResult result) {
		int position = component.getPosition();
		Usage usage = component.getUsage();
		Ref ref = component.getRef();
		...
		this.childComplianceMap.put(positionPath,
				new ComplianceObject(positionPath, path, usage, null, null, minLength, maxLength));
		this.checkUsageComplianceError(conformanceProfile, dt.getId(), dt.getType(), new DTSegMetadata(dt), usage,
				positionPath, path, result);
		this.checkLengthComplianceError(conformanceProfile, dt.getId(), dt.getType(), new DTSegMetadata(dt), minLength,
				maxLength, positionPath, path, result);
		...
		if (ref != null && ref.getId() != null) {
			...
			if (datatype != null) {
				if (datatype instanceof ComplexDatatype) {
					...
					if (cDT.getComponents() != null && cDT.getComponents().size() > 0) {
						for (Component childComponent : cDT.getComponents()) {
							this.checkComponentForConformanceProfileForCompliance(conformanceProfile, cDT,
									childComponent, positionPath, path, result);
						}
					}
				}
	...
	private void checkCardinalityComplianceError(ConformanceProfile conformanceProfile, String target, Type targetType,
			Object targetMeta, int min, String max, String positionPath, String path, CPComplianceResult result) {
		ComplianceObject complianceParentObject = this.parentComplianceMap.get(positionPath);
		if (max != null) {
			if (this.isInt(max)) {
				if (complianceParentObject != null && complianceParentObject.getMax() != null
						&& this.isInt(complianceParentObject.getMax())) {
					...
					if (parentMaxInt < childMaxInt) {
						result.getErrors().add(new IgamtObjectError("MAX_Cardinality_Compliance", target, targetType,
								targetMeta,
								"In " + conformanceProfile.getLabel() + ", " + path + " assigned Max cardinality of "
										+ childMaxInt + " is not compliant with Max cardinality of " + parentMaxInt,
								positionPath + "", "WARNING", "User"));
					}
				}
			}
		}
		if (complianceParentObject != null && complianceParentObject.getMin() != null
				&& complianceParentObject.getMin() > min) {
			result.getErrors()
					.add(new IgamtObjectError("MIN_Cardinality_Compliance", target, targetType, targetMeta,
							"In " + conformanceProfile.getLabel() + ", " + path + " assigned Min cardinality of " + min
									+ " is not compliant with Min cardinality of " + complianceParentObject.getMin(),
							positionPath + "", "WARNING", "User"));
		}
	}

	private void checkCardinalityVerificationError(String target, Type targetType, Object targetMeta, Usage usage,
			int min, String max, String positionPath, String path, VerificationResult result) {
		if (max == null) {
			result.getErrors().add(new IgamtObjectError("Cardinality_MISSING", target, targetType, targetMeta,
					"In " + path + ", Cardinality is missing", positionPath + "", "ERROR", "User"));
		} else {
			...
			if (this.isIntOrStar(max)) {
				if (this.isInt(max)) {
					...
					if (min > maxInt) {
						result.getErrors().add(new IgamtObjectError("Cardinality_Range", target, targetType, targetMeta,
								"In " + path
										+ ", MIN Cardinality value is bigger than MAX Cardinality. Current MinCardinality is "
										+ min + " and Current MaxCardinality is " + max,
								positionPath + "", "ERROR", "User"));
					} else {
						if (usage != null && usage.equals(Usage.X) && !max.equals("0")) {
							result.getErrors().add(new IgamtObjectError("MAX_Cardinality_UsageX", target, targetType,
									targetMeta,
									"In " + path
											+ ", MAX cardinality value should be 0, if the USAGE is X. Current MaxCardinality is "
											+ max,
									positionPath + "", "WARNING", "User"));
						}

						if (usage != null && usage.equals(Usage.R) && min < 1) {
							result.getErrors().add(new IgamtObjectError("MIN_Cardinality_UsageR", target, targetType,
									targetMeta,
									"In " + path
											+ ", MIN cardinality value should be bigger than 0, if the USAGE is R.",
									positionPath + "", "WARNING", "User"));
						}

						if (usage != null && !usage.equals(Usage.R) && min != 0) {
							result.getErrors().add(new IgamtObjectError("MIN_Cardinality_UsageNotR", target, targetType,
									targetMeta,
									"In " + path
											+ ", MIN cardinality value should be 0, if the USAGE is not R. Current usage is "
											+ usage + " and current MinCardinality is " + min,
									positionPath + "", "WARNING", "User"));
						}
					}
				}
				...
				result.getErrors()
						.add(new IgamtObjectError("MAX_Cardinality_Format", target, targetType, targetMeta, "In " + path
								+ ", MAX cardinality value should be number or '*'.  Current MaxCardinality is " + max,
								positionPath + "", "ERROR", "User"));
			}
		}
	}

	private void checkUsageComplianceError(ConformanceProfile conformanceProfile, String target, Type targetType,
			Object targetMeta, Usage usage, String positionPath, String path, CPComplianceResult result) {
		if (usage != null) {
				...
				if (complianceParentObject != null && complianceParentObject.getUsage() != null) {
					...
					if (parentUsage.equals(Usage.R)) {
						if (!usage.equals(Usage.R)) {
							result.getErrors()
									.add(new IgamtObjectError("Usage_Compliance", target, targetType, targetMeta,
											"In " + conformanceProfile.getLabel() + ", " + path + " assigned usage of "
													+ usage + " is not compliant with parent usage of " + parentUsage,
											positionPath + "", "WARNING", "User"));
						}
						...
						if (!usage.equals(Usage.R) && !usage.equals(Usage.RE) && !usage.equals(Usage.C)
								&& !usage.equals(Usage.CAB) && !usage.equals(Usage.X)) {
							result.getErrors()
									.add(new IgamtObjectError("Usage_Compliance", target, targetType, targetMeta,
											"In " + conformanceProfile.getLabel() + ", " + path + " assigned usage of "
													+ usage + " is not compliant with parent usage of " + parentUsage,
						...
						if (!usage.equals(Usage.R) && !usage.equals(Usage.RE) && !usage.equals(Usage.CAB)
								&& !usage.equals(Usage.O) && !usage.equals(Usage.X) && !usage.equals(Usage.B)) {
							result.getErrors()
									.add(new IgamtObjectError("Usage_Compliance", target, targetType, targetMeta,
											"In " + conformanceProfile.getLabel() + ", " + path + " assigned usage of "
													+ usage + " is not compliant with parent usage of " + parentUsage,
	...
	private void checkUsageVerificationError(ConformanceProfile conformanceProfile, String target, Type targetType,
			Object targetMeta, Usage usage, String positionPath, String path, VerificationResult result) {
		// String code, String target, String targetType, String description,String
		// location, String severity
		if (usage == null) {
			result.getErrors().add(new IgamtObjectError("Usage_MISSING", target, targetType, targetMeta,
					"In " + path + ", Usage is missing", positionPath + "", "FATAL", "User"));
		}
	}
	...
	private void checkingGroupForCompliance(ConformanceProfile conformanceProfile, Group group,
			CPComplianceResult result, String positionPath, String path, Set<StructureElementBinding> sebs) {
		...
		if (this.isNotNullNotEmpty(name)) {
			...
			this.checkUsageComplianceError(conformanceProfile, conformanceProfile.getId(), conformanceProfile.getType(),
					new CPMetadata(conformanceProfile), usage, positionPath, path, result);
			this.checkCardinalityComplianceError(conformanceProfile, conformanceProfile.getId(),
					conformanceProfile.getType(), new CPMetadata(conformanceProfile), min, max, positionPath, path,
					result);
			...
			if (group.getChildren() != null) {
				for (SegmentRefOrGroup child : group.getChildren()) {
					this.checkingSegmentRefOrGroupForCompliance(conformanceProfile, child, result, positionPath, path,
							this.findSEB(sebs, child.getId()));
				}
			}
		}
	}
	...
	private void checkingGroup(ConformanceProfile conformanceProfile, Group group, CPVerificationResult result,
		...
		if (!this.isNotNullNotEmpty(name)) {
			result.getErrors()
					.add(new IgamtObjectError("Name_Missing", conformanceProfile.getId(), conformanceProfile.getType(),
							new CPMetadata(conformanceProfile), "name is missing", positionPath + "", "ERROR", "User"));
		} else {
			if (path == null)
				path = name;
			else
	...
	private void checkingFields(Segment segment, Set<Field> fields, DTSegVerificationResult result) {
		if (fields == null || fields.size() == 0) {
			result.getErrors().add(new IgamtObjectError("Field_Missing", segment.getId(), segment.getType(),
					new DTSegMetadata(segment), "Field is missing for Segment.", null, "FATAL", "User"));
		} else {
		...
		if (usage == null) {
			result.getErrors()
					.add(new IgamtObjectError("Code_Usage_Missing", valueset.getId(), Type.VALUESET,
							new VSMetadata(valueset), "In code:" + value + ", the USAGE is missing", c.getValue(),
							"ERROR", "User"));
		} else {
			if (extensibility != null && extensibility.equals(Extensibility.Closed)) {
				if (usage == null || usage.equals(CodeUsage.P)) {
					result.getErrors()
							.add(new IgamtObjectError("Code_Usage_Value_ClosedValueset", valueset.getId(),
									Type.VALUESET, new VSMetadata(valueset), "In code: " + value
											+ ", the Usage must be one of R/E, but current Usage is " + usage,
									c.getValue(), "WARNING", "User"));
				}
			}
		}
		...
		valueset.getCodes().forEach(otherC -> {
			if (!otherC.getId().equals(c.getId())) {
				if ((otherC.getValue() + "-" + otherC.getCodeSystem()).equals(c.getValue() + "-" + c.getCodeSystem()))
					result.getErrors()
							.add(new IgamtObjectError("Value-Codesys_Duplicated", valueset.getId(), Type.VALUESET,
									new VSMetadata(valueset), "the combination of value (" + c.getValue()
											+ ") and codesys (" + c.getCodeSystem() + ") are duplicated.",
									c.getValue(), "ERROR", "User"));
			}
		});
	}
	...
	private void checkingComponent(ComplexDatatype cDt, Component c, DTSegVerificationResult result) {
		...
		if (position == 0) {
			result.getErrors().add(new IgamtObjectError("Position_Missing", cDt.getId(), cDt.getType(),
					new DTSegMetadata(cDt), "Position is missing", position + "", "FATAL", "Internal"));
		} else {
			String path = cDt.getLabel() + "." + position;
			if (!this.isNotNullNotEmpty(name))
				result.getErrors().add(new IgamtObjectError("In " + path + ", Name_Missing", cDt.getId(), cDt.getType(),
						new DTSegMetadata(cDt), "name is missing", position + "", "FATAL", "User"));
			if (ref == null || ref.getId() == null)
				result.getErrors()
						.add(new IgamtObjectError("REF_Missing", cDt.getId(), cDt.getType(), new DTSegMetadata(cDt),
								"In " + path + ", Ref value is missing.", position + "", "FATAL", "User"));
			if (type == null || !type.equals(Type.COMPONENT))
				result.getErrors()
						.add(new IgamtObjectError("Type_Missing", cDt.getId(), cDt.getType(), new DTSegMetadata(cDt),
								"In " + path + ", the Type is missing", position + "", "FATAL", "Internal"));

			this.checkUsageVerificationError(null, cDt.getId(), cDt.getType(), new DTSegMetadata(cDt), usage,
					position + "", cDt.getLabel() + "." + position, result);
			this.checkLengthVerificationErorr(cDt.getId(), cDt.getType(), new DTSegMetadata(cDt), c, minLength,
					maxLength, confLength, "", cDt.getLabel() + "." + position, result);

			cDt.getComponents().forEach(otherC -> {
				if (!otherC.getId().equals(c.getId())) {
					if (otherC.getPosition() == position)
						result.getErrors()
								.add(new IgamtObjectError("Position_Dupilicated", cDt.getId(), cDt.getType(),
										new DTSegMetadata(cDt), "The position:" + position + " is duplicated.",
										position + "", "FATAL", "Internal"));
//          if (otherC.getName().equals(c.getName())) result.getErrors().add(new IgamtObjectError("Name_Duplicated", cDt.getId(), cDt.getType(), new DTSegMetadata(cDt), "name is duplicated for " + position + " and " + otherC.getPosition(), position + "", "ERROR", "User"));
				...
				if (childDt == null)
					result.getErrors()
							.add(new IgamtObjectError("Ref_NotAccessable", cDt.getId(), cDt.getType(),
									new DTSegMetadata(cDt), "In " + path + " Ref object is not accesable.",
									position + "", "ERROR", "Internal"));

				if (this.isPrimitiveDatatype(childDt)) {

				} else {
					if (this.isNotNullNotEmpty(c.getConstantValue()))
						result.getErrors()
								.add(new IgamtObjectError("Constant_NOTAllowed", cDt.getId(), cDt.getType(),
										new DTSegMetadata(cDt), "In " + path + " , Constant value is not allowed",
										position + "", "ERROR", "User"));
				}
	...
	private void checkingField(Segment segment, Field f, DTSegVerificationResult result) {
		...
		if (position == 0) {
			result.getErrors().add(new IgamtObjectError("Position_Missing", segment.getId(), segment.getType(),
					new DTSegMetadata(segment), "Position is missing", position + "", "FATAL", "Internal"));
		} else {
			String path = segment.getLabel() + "-" + position;
			if (!this.isNotNullNotEmpty(name))
				result.getErrors()
						.add(new IgamtObjectError("In " + path + ", Name_Missing", segment.getId(), segment.getType(),
								new DTSegMetadata(segment), "name is missing", position + "", "FATAL", "User"));
			if (ref == null || ref.getId() == null)
				result.getErrors()
						.add(new IgamtObjectError("REF_Missing", segment.getId(), segment.getType(),
								new DTSegMetadata(segment), "In " + path + ", Ref value is missing.", position + "",
								"FATAL", "User"));
			if (type == null || !type.equals(Type.FIELD))
				result.getErrors()
						.add(new IgamtObjectError("Type_Missing", segment.getId(), segment.getType(),
								new DTSegMetadata(segment), "In " + path + ", the Type is missing", position + "",
								"FATAL", "Internal"));

			this.checkUsageVerificationError(null, segment.getId(), segment.getType(), new DTSegMetadata(segment),
					usage, position + "", segment.getLabel() + "-" + position, result);
			this.checkCardinalityVerificationError(segment.getId(), segment.getType(), new DTSegMetadata(segment),
					usage, min, max, position + "", segment.getLabel() + "-" + position, result);
			this.checkLengthVerificationErorr(segment.getId(), segment.getType(), new DTSegMetadata(segment), f,
					minLength, maxLength, confLength, position + "", segment.getLabel() + "-" + position, result);

			segment.getChildren().forEach(otherF -> {
				if (!otherF.getId().equals(f.getId())) {
					if (otherF.getPosition() == position)
						result.getErrors()
								.add(new IgamtObjectError("Position_Dupilicated", segment.getId(), segment.getType(),
										new DTSegMetadata(segment), "The position:" + position + " is duplicated.",
										position + "", "FATAL", "Internal"));
//          if (otherF.getName().equals(f.getName())) result.getErrors().add(new IgamtObjectError("Name_Duplicated", segment.getId(), segment.getType(), new DTSegMetadata(segment), "name is duplicated for " + position + " and " + otherF.getPosition(), position + "", "ERROR", "User"));
				...
				if (childDt == null)
					result.getErrors()
							.add(new IgamtObjectError("Ref_NotAccessable", segment.getId(), segment.getType(),
									new DTSegMetadata(segment), "In " + path + " Ref object is not accesable.",
									position + "", "ERROR", "Internal"));

				if (this.isPrimitiveDatatype(childDt)) {

				} else {
					if (this.isNotNullNotEmpty(f.getConstantValue()))
						result.getErrors()
								.add(new IgamtObjectError("Constant_NOTAllowed", segment.getId(), segment.getType(),
										new DTSegMetadata(segment), "In " + path + " , Constant value is not allowed",
										position + "", "ERROR", "User"));
				}
	...
	private void checkingInternalSingleCode(ComplexDatatype cDt, InternalSingleCode internalSingleCode,
			DTSegVerificationResult result, int position, String path) {
		...
		if (!this.isNotNullNotEmpty(code))
			result.getErrors()
					.add(new IgamtObjectError("SingleCode_Code_Missing", cDt.getId(), cDt.getType(),
							new DTSegMetadata(cDt), "In " + path + ", Code is missing for SingleCode.", position + "",
							"ERROR", "User"));
		if (!this.isNotNullNotEmpty(codeSystem))
			result.getErrors()
					.add(new IgamtObjectError("SingleCode_CodeSys_Missing", cDt.getId(), cDt.getType(),
							new DTSegMetadata(cDt), "In " + path + ", CodeSystem is missing for SingleCode.",
							position + "", "ERROR", "User"));
		if (!this.isNotNullNotEmpty(valueSetId))
			result.getErrors()
					.add(new IgamtObjectError("SingleCode_Valueset_Missing", cDt.getId(), cDt.getType(),
							new DTSegMetadata(cDt), "In " + path + ", ValueSet is missing for SingleCode.",
							position + "", "ERROR", "User"));

		if (valueSetId != null) {
			if (this.valuesetService.findById(valueSetId) == null)
				result.getErrors()
						.add(new IgamtObjectError("SingleCode_Valueset_NotAccessable", cDt.getId(), cDt.getType(),
								new DTSegMetadata(cDt), "In " + path + ", ValueSet is not accessable for SingleCode.",
								position + "", "ERROR", "Internal"));
		}
	}

	private void checkingInternalSingleCode(Segment segment, InternalSingleCode internalSingleCode,
			DTSegVerificationResult result, int position, String path) {
		String code = internalSingleCode.getCode();
		String codeSystem = internalSingleCode.getCodeSystem();
		String valueSetId = internalSingleCode.getValueSetId();
		...
		if (!this.isNotNullNotEmpty(code))
			result.getErrors()
					.add(new IgamtObjectError("SingleCode_Code_Missing", segment.getId(), segment.getType(),
							new DTSegMetadata(segment), "In " + path + ", Code is missing for SingleCode.",
							position + "", "ERROR", "User"));
		if (!this.isNotNullNotEmpty(codeSystem))
			result.getErrors()
					.add(new IgamtObjectError("SingleCode_CodeSys_Missing", segment.getId(), segment.getType(),
							new DTSegMetadata(segment), "In " + path + ", CodeSystem is missing for SingleCode.",
							position + "", "ERROR", "User"));
		if (!this.isNotNullNotEmpty(valueSetId))
			result.getErrors()
					.add(new IgamtObjectError("SingleCode_Valueset_Missing", segment.getId(), segment.getType(),
							new DTSegMetadata(segment), "In " + path + ", ValueSet is missing for SingleCode.",
							position + "", "ERROR", "User"));

		if (valueSetId != null) {
			if (this.valuesetService.findById(valueSetId) == null)
				result.getErrors()
						.add(new IgamtObjectError("SingleCode_Valueset_NotAccessable", segment.getId(),
								segment.getType(), new DTSegMetadata(segment),
								"In " + path + ", ValueSet is not accessable for SingleCode.", position + "", "ERROR",
								"Internal"));
		}
	}

	private void checkingExternalSingleCode(ComplexDatatype cDt, ExternalSingleCode externalSingleCode,
			DTSegVerificationResult result, int position, String path) {
		...
		if (!this.isNotNullNotEmpty(value))
			result.getErrors()
					.add(new IgamtObjectError("SingleCode_Value_Missing", cDt.getId(), cDt.getType(),
							new DTSegMetadata(cDt), "In " + path + ", Value is missing for SingleCode.", position + "",
							"ERROR", "User"));
		if (!this.isNotNullNotEmpty(codeSystem))
			result.getErrors()
					.add(new IgamtObjectError("SingleCode_CodeSys_Missing", cDt.getId(), cDt.getType(),
	...
	private void checkingExternalSingleCode(Segment segment, ExternalSingleCode externalSingleCode,
			DTSegVerificationResult result, int position, String path) {
		String value = externalSingleCode.getValue();
		String codeSystem = externalSingleCode.getCodeSystem();
		...
		if (!this.isNotNullNotEmpty(value))
			result.getErrors()
					.add(new IgamtObjectError("SingleCode_Value_Missing", segment.getId(), segment.getType(),
							new DTSegMetadata(segment), "In " + path + ", Value is missing for SingleCode.",
							position + "", "ERROR", "User"));
		if (!this.isNotNullNotEmpty(codeSystem))
			result.getErrors()
					.add(new IgamtObjectError("SingleCode_CodeSys_Missing", segment.getId(), segment.getType(),
	...
	private void checkingValueSetBindings(ComplexDatatype cDt, Set<ValuesetBinding> valuesetBindings,
			DTSegVerificationResult result, int position, String path) {
		for (ValuesetBinding vb : valuesetBindings) {
			for (String vsId : vb.getValueSets()) {
				if (vsId == null) {
					result.getErrors()
							.add(new IgamtObjectError("ValueSetBinding_VSID_Missing", cDt.getId(), cDt.getType(),
									new DTSegMetadata(cDt),
									"In " + path + ", Valueset Id is missing for ValueSet Binding.", position + "",
									"ERROR", "User"));
				} else {
					if (this.valuesetService.findById(vsId) == null)
						result.getErrors()
								.add(new IgamtObjectError("ValueSetBinding_VS_NotAccessable", cDt.getId(),
										cDt.getType(), new DTSegMetadata(cDt),
										"In " + path + ", Valueset is not accessable for ValueSet Binding.",
										position + "", "ERROR", "Internal"));
				}
			}
		}
	...
	private void checkingValueSetBindings(Segment segment, Set<ValuesetBinding> valuesetBindings,
			DTSegVerificationResult result, int position, String path) {
		for (ValuesetBinding vb : valuesetBindings) {
			for (String vsId : vb.getValueSets()) {
				if (vsId == null) {
					result.getErrors()
							.add(new IgamtObjectError("ValueSetBinding_VSID_Missing", segment.getId(),
									segment.getType(), new DTSegMetadata(segment),
									"In " + path + ", Valueset Id is missing for ValueSet Binding.", position + "",
									"ERROR", "User"));
				} else {
					if (this.valuesetService.findById(vsId) == null)
						result.getErrors()
								.add(new IgamtObjectError("ValueSetBinding_VS_NotAccessable", segment.getId(),
										segment.getType(), new DTSegMetadata(segment),
										"In " + path + ", Valueset is not accessable for ValueSet Binding.",
										position + "", "ERROR", "Internal"));
				}
...
//  }
	...
	private void checkingMetadataForValueset(Valueset valueset, VSVerificationResult result) {
		if (valueset == null) {
			...
			if (!this.isNotNullNotEmptyNotWhiteSpaceOnly(bId))
				result.getErrors().add(new IgamtObjectError("BindingId_Missing", valueset.getId(), valueset.getType(),
						new VSMetadata(valueset), "Valueset binding Identifier is missing", null, "FATAL", "User"));
			if (name == null)
				result.getErrors().add(new IgamtObjectError("Name_Missing", valueset.getId(), valueset.getType(),
						new VSMetadata(valueset), "name is missing", null, "FATAL", "User"));
//      if (description == null) result.getErrors().add(new IgamtObjectError("Desc_Missing", null, null, "Description is missing", null, "WARNING"));
			...
			if (extensibility == null)
				result.getErrors()
						.add(new IgamtObjectError("Extensibility_Missing", valueset.getId(), valueset.getType(),
								new VSMetadata(valueset), "Extensibility is missing", null, "WARNING", "User"));
			if (stability == null)
				result.getErrors().add(new IgamtObjectError("Stability_Missing", valueset.getId(), valueset.getType(),
						new VSMetadata(valueset), "Stability is missing", null, "WARNING", "User"));
			if (contentDefinition == null)
				result.getErrors()
						.add(new IgamtObjectError("ContentDefinition_Missing", valueset.getId(), valueset.getType(),
								new VSMetadata(valueset), "ContentDefinition is missing", null, "WARNING", "User"));

			if (domainInfo == null || domainInfo.getScope() == null) {
				result.getErrors().add(new IgamtObjectError("Scope_Missing", valueset.getId(), valueset.getType(),
						new VSMetadata(valueset), "Scope is missing", null, "ERROR", "Internal"));
			}
			if (domainInfo == null || domainInfo.getVersion() == null) {
				result.getErrors().add(new IgamtObjectError("Version_Missing", valueset.getId(), valueset.getType(),
						new VSMetadata(valueset), "Version is missing", null, "WARNING", "Internal"));
			}

			if (valueset.getCodes() != null && valueset.getCodes().size() > 500) {
				result.getErrors()
						.add(new IgamtObjectError("CodeSet_Size_Exceeded", valueset.getId(), valueset.getType(),
								new VSMetadata(valueset), "IGAMT imposed code set limit (500) exceeded.", null,
								"WARNING", "Internal"));
			}
		}
	}
	...
	private void checkingMetadataForDatatype(Datatype datatype, DTSegVerificationResult result) {
		if (datatype == null) {
			...
			if (!this.isNotNullNotEmptyNotWhiteSpaceOnly(name)) {
				result.getErrors().add(new IgamtObjectError("Name_Missing", datatype.getId(), datatype.getType(),
						new DTSegMetadata(datatype), "name is missing", null, "FATAL", "User"));
			}
			if (description == null)
				result.getErrors().add(new IgamtObjectError("Desc_Missing", datatype.getId(), datatype.getType(),
						new DTSegMetadata(datatype), "Description is missing", null, "WARNING", "User"));

			if (domainInfo == null || domainInfo.getScope() == null) {
				result.getErrors().add(new IgamtObjectError("Scope_Missing", datatype.getId(), datatype.getType(),
						new DTSegMetadata(datatype), "Scope is missing", null, "ERROR", "Internal"));
			}
			if (domainInfo == null || domainInfo.getVersion() == null) {
				result.getErrors().add(new IgamtObjectError("Version_Missing", datatype.getId(), datatype.getType(),
						new DTSegMetadata(datatype), "Version is missing", null, "WARNING", "Internal"));
			}

			if (domainInfo != null && domainInfo.getScope() != null && domainInfo.getScope().equals(Scope.USER)
					&& !this.isNotNullNotEmptyNotWhiteSpaceOnly(ext)) {
				result.getErrors()
						.add(new IgamtObjectError("EXT_Missing", datatype.getId(), datatype.getType(),
								new DTSegMetadata(datatype), "EXTENSION is missing, but scope is users", null, "FATAL",
								"User"));
			}
			...
			if (ext != null) {
				...
				if (!matcher.matches() || ext.length() > 4) {
					result.getErrors()
							.add(new IgamtObjectError("EXT_Format", datatype.getId(), datatype.getType(),
									new DTSegMetadata(datatype),
									"User extension should start with a letter and be within 4 characters long.", null,
									"WARNING", "User"));
				}
			}
		}
	...
	private void checkingMetadataForSegment(Segment segment, DTSegVerificationResult result) {
		if (segment == null) {
			...
			if (!this.isNotNullNotEmptyNotWhiteSpaceOnly(name)) {
				result.getErrors().add(new IgamtObjectError("Name_Missing", segment.getId(), segment.getType(),
						new DTSegMetadata(segment), "name is missing", null, "FATAL", "User"));
			}
			if (description == null)
				result.getErrors().add(new IgamtObjectError("Desc_Missing", segment.getId(), segment.getType(),
						new DTSegMetadata(segment), "Description is missing", null, "WARNING", "User"));

			if (domainInfo == null || domainInfo.getScope() == null) {
				result.getErrors().add(new IgamtObjectError("Scope_Missing", segment.getId(), segment.getType(),
						new DTSegMetadata(segment), "Scope is missing", null, "ERROR", "Internal"));
			}
			if (domainInfo == null || domainInfo.getVersion() == null) {
				result.getErrors().add(new IgamtObjectError("Version_Missing", segment.getId(), segment.getType(),
						new DTSegMetadata(segment), "Version is missing", null, "WARNING", "Internal"));
			}

			if (domainInfo != null && domainInfo.getScope() != null && domainInfo.getScope().equals(Scope.USER)
					&& !this.isNotNullNotEmptyNotWhiteSpaceOnly(ext)) {
				result.getErrors().add(new IgamtObjectError("EXT_Missing", segment.getId(), segment.getType(),
						new DTSegMetadata(segment), "EXTENSION is missing, but scope is users", null, "FATAL", "User"));
			}
			...
			if (ext != null) {
				...
				if (!matcher.matches() || ext.length() > 4) {
					result.getErrors()
							.add(new IgamtObjectError("EXT_Format", segment.getId(), segment.getType(),
	...
	private void checkingMetadataForConformanceProfile(ConformanceProfile conformanceProfile,
		...
		if (conformanceProfile == null) {
			...
			if (!this.isNotNullNotEmptyNotWhiteSpaceOnly(name)) {
				result.getErrors()
						.add(new IgamtObjectError("Name_Missing", conformanceProfile.getId(),
								conformanceProfile.getType(), new CPMetadata(conformanceProfile), "name is missing",
								null, "FATAL", "User"));
			}

			if (description == null)
				result.getErrors()
						.add(new IgamtObjectError("Desc_Missing", conformanceProfile.getId(),
								conformanceProfile.getType(), new CPMetadata(conformanceProfile),
								"Description is missing", null, "WARNING", "User"));

			if (!this.isNotNullNotEmptyNotWhiteSpaceOnly(structId)) {
				result.getErrors()
						.add(new IgamtObjectError("StructId_Missing", conformanceProfile.getId(),
								conformanceProfile.getType(), new CPMetadata(conformanceProfile), "StructId is missing",
								null, "FATAL", "Internal"));
			}

			if (!this.isNotNullNotEmptyNotWhiteSpaceOnly(mType)) {
				result.getErrors()
						.add(new IgamtObjectError("MessageType_Missing", conformanceProfile.getId(),
								conformanceProfile.getType(), new CPMetadata(conformanceProfile),
								"Message Type is missing.", null, "FATAL", "Internal"));
			}

			if (!this.isNotNullNotEmptyNotWhiteSpaceOnly(event)) {
				result.getErrors()
						.add(new IgamtObjectError("TriggerEvent_Missing", conformanceProfile.getId(),
								conformanceProfile.getType(), new CPMetadata(conformanceProfile),
								"Trigger Event is missing.", null, "FATAL", "Internal"));
			}

			if (domainInfo == null || domainInfo.getScope() == null) {
				result.getErrors()
						.add(new IgamtObjectError("Scope_Missing", conformanceProfile.getId(),
								conformanceProfile.getType(), new CPMetadata(conformanceProfile), "Scope is missing",
								null, "ERROR", "Internal"));
			}
			if (domainInfo == null || domainInfo.getVersion() == null) {
				result.getErrors()
						.add(new IgamtObjectError("Version_Missing", conformanceProfile.getId(),
								conformanceProfile.getType(), new CPMetadata(conformanceProfile), "Version is missing",
								null, "WARNING", "Internal"));
			}
		}
	}
	...
	private void checkingDynamicMapping(Segment segment, DTSegVerificationResult result) {
		if (this.isAvaliableDynamicMappingSegmnet(segment)) {
			Valueset vs = findValuesetByFieldPath(segment, 2);
			if (vs == null)
				result.getErrors()
				...
				if (dynamicMappingInfo != null && dynamicMappingInfo.getItems() != null) {
					for(Code code : vs.getCodes()) {
						if (!dynamicMappingInfo.contains(code)) {
							result.getErrors()
	...
	private Valueset findValuesetByFieldPath(Segment segment, int i) {
		for (StructureElementBinding seb : segment.getBinding().getChildren()) {
			if (seb.getElementId().equals("" + i)) {
	...
	private CPComplianceResult verifyConformanceProfileForCompliance(ConformanceProfile conformanceProfile) {
		...
		if (conformanceProfile.getOrigin() != null) {
			ConformanceProfile parentConformanceProfile = this.conformanceProfileService
					.findById(conformanceProfile.getOrigin());
			Set<StructureElementBinding> sebs = null;
			if (parentConformanceProfile.getBinding() != null)
				sebs = parentConformanceProfile.getBinding().getChildren();
			this.travelSegmentRefOrGroups(parentConformanceProfile, parentConformanceProfile.getChildren(), null, null,
					sebs);
		}
	 ...
	 * @param sebs
	 */
	private void travelSegmentRefOrGroups(ConformanceProfile conformanceProfile,
			Set<SegmentRefOrGroup> segmentRefOrGroups, String positionPath, String path,
			Set<StructureElementBinding> sebs) {
		if (segmentRefOrGroups != null) {
			segmentRefOrGroups.forEach(srog -> this.travelSegmentRefOrGroup(conformanceProfile, srog, positionPath,
					path, this.findSEB(sebs, srog.getId())));
		}
	}
	 ...
	 * @return
	 */
	private void travelSegmentRefOrGroup(ConformanceProfile conformanceProfile, SegmentRefOrGroup srog,
			String positionPath, String path, Set<StructureElementBinding> sebs) {
		if (srog instanceof SegmentRef) {
	 ...
	 * @param sebs
	 */
	private void travelGroup(ConformanceProfile conformanceProfile, Group group, String positionPath, String path,
			Set<StructureElementBinding> sebs) {
		String name = group.getName();
		int position = group.getPosition();
		Usage usage = group.getUsage();
		...
		if (!this.isNotNullNotEmpty(name)) {
			...
			this.parentComplianceMap.put(positionPath,
					new ComplianceObject(positionPath, path, usage, min, max, null, null));

			if (group.getChildren() != null) {
				for (SegmentRefOrGroup child : group.getChildren()) {
					this.travelSegmentRefOrGroup(conformanceProfile, child, positionPath, path,
							this.findSEB(sebs, child.getId()));
				}
			}
		}
	 ...
	 * @param sebs
	 */
	private void travelSegmentRef(ConformanceProfile conformanceProfile, SegmentRef sr, String positionPath,
			String path, Set<StructureElementBinding> sebs) {
		int position = sr.getPosition();
		Usage usage = sr.getUsage();
		int min = sr.getMin();
		String max = sr.getMax();
	 ...
	 * @param path
	 */
	private void travelFieldForConformanceProfile(ConformanceProfile conformanceProfile, Field field,
		...
		if (ref != null && ref.getId() != null) {
			...
			if (datatype != null) {
				if (datatype instanceof ComplexDatatype) {
					...
					if (cDT.getComponents() != null && cDT.getComponents().size() > 0) {
						for (Component component : cDT.getComponents()) {
							this.travelComponentForConformanceProfile(conformanceProfile, component, positionPath,
									path);
						}
					}
				}
	 ...
	 * @param path
	 */
	private void travelComponentForConformanceProfile(ConformanceProfile conformanceProfile, Component component,
			String positionPath, String path) {
		int position = component.getPosition();
		Usage usage = component.getUsage();
		Ref ref = component.getRef();
		...
		if (ref != null && ref.getId() != null) {
			...
			if (datatype != null) {
				if (datatype instanceof ComplexDatatype) {
					...
					if (cDT.getComponents() != null && cDT.getComponents().size() > 0) {
						for (Component childComponent : cDT.getComponents()) {
							this.travelComponentForConformanceProfile(conformanceProfile, childComponent, positionPath,
									path);
						}
					}
				}
	...
	public VerificationReport verifyIg(Ig ig, boolean needDeep) {
		...
		if (valueSetRegistry.getChildren() != null) {
			for (Link l : valueSetRegistry.getChildren()) {
				...
				if (vs == null)
					result.getErrors()
							.add(new IgamtObjectError("Link_NotAccessable", id, Type.VALUESET, null,
									"In valueset Repository, valueset : " + id + " is not accesable", null, "ERROR",
									"Internal"));
				else {
					...
					if (!bIdSet.add(bindingIdWithVersion))
						result.getErrors().add(new IgamtObjectError("BindingId_Duplicated", vs.getId(), Type.VALUESET,
								new VSMetadata(vs),
								"Binding Identifier of " + vs.getBindingIdentifier() + " is duplicated in the IG", null,
								"ERROR", "User"));
					if (vs.getDomainInfo().getScope() == null
							|| !vs.getDomainInfo().getScope().equals(domainInfo.getScope()))
						result.getErrors().add(new IgamtObjectError("Link_Scope_notMatched", vs.getId(), Type.VALUESET,
								new VSMetadata(vs),
								"In valueset Repository, " + vs.getLabel()
										+ "'s scope value of the Link is not matched with actual Valueset's scope.",
								null, "ERROR", "Internal"));
					if (vs.getDomainInfo().getVersion() != null
							&& !vs.getDomainInfo().getVersion().equals(domainInfo.getVersion()))
						result.getErrors().add(new IgamtObjectError("Link_Version_notMatched", vs.getId(),
								Type.VALUESET, new VSMetadata(vs),
								"In valueset Repository, " + vs.getLabel()
										+ "'s version value of the Link is not matched with actual Valueset's version.",
								null, "ERROR", "Internal"));
				...
				if (dt == null)
					result.getErrors()
							.add(new IgamtObjectError("Link_NotAccessable", id, Type.DATATYPE, null,
									"In dataytpe Repository, datatype : " + id + " is not accesable", null, "ERROR",
									"Internal"));
				else {
					...
					if (!dtLabelSet.add(labelWithVersion))
						result.getErrors()
								.add(new IgamtObjectError("Label_Duplicated", dt.getId(), Type.DATATYPE,
										new DTSegMetadata(dt),
										"Datatype Label of " + dt.getLabel() + " is duplicated in the IG", null,
										"ERROR", "User"));
					if (dt.getDomainInfo().getScope() == null
							|| !dt.getDomainInfo().getScope().equals(domainInfo.getScope()))
						result.getErrors().add(new IgamtObjectError("Link_Scope_notMatched", dt.getId(), Type.DATATYPE,
								new DTSegMetadata(dt),
								"In datatype Repository, " + dt.getLabel()
										+ "'s scope value of the Link is not matched with actual datatype's scope.",
								null, "ERROR", "Internal"));
					if (dt.getDomainInfo().getVersion() == null
							|| !dt.getDomainInfo().getVersion().equals(domainInfo.getVersion()))
						result.getErrors().add(new IgamtObjectError("Link_Version_notMatched", dt.getId(),
								Type.DATATYPE, new DTSegMetadata(dt),
								"In datatype Repository, " + dt.getLabel()
										+ "'s version value of the Link is not matched with actual datatype's version.",
								null, "ERROR", "Internal"));
				...
				if (s == null)
					...
					if (!segLabelSet.add(labelWithVersion))
						result.getErrors().add(new IgamtObjectError("Label_Duplicated", s.getId(), Type.SEGMENT,
								...
								null, "ERROR", "User"));
					if (s.getDomainInfo().getScope() == null
							|| !s.getDomainInfo().getScope().equals(domainInfo.getScope()))
						result.getErrors().add(new IgamtObjectError("Link_Scope_notMatched", s.getId(), Type.SEGMENT,
								new DTSegMetadata(s),
								"In segment Repository, " + s.getLabel()
										+ "'s scope value of the Link is not matched with actual segment scope.",
								null, "ERROR", "Internal"));
					if (s.getDomainInfo().getVersion() != null
							&& !s.getDomainInfo().getVersion().equals(domainInfo.getVersion()))
						result.getErrors().add(new IgamtObjectError("Link_Version_notMatched", s.getId(), Type.SEGMENT,
								new DTSegMetadata(s),
								"In segment Repository, " + s.getLabel()
										+ "'s version value of the Link is not matched with actual segment version.",
								null, "ERROR", "Internal"));
			...
			for (Link l : conformanceProfileRegistry.getChildren()) {
				...
				if (cp == null)
					result.getErrors().add(new IgamtObjectError("Link_NotAccessable", id, Type.CONFORMANCEPROFILE, null,
							"In conformance profile Repository, conformance profile : " + id + " is not accesable",
							null, "ERROR", "Internal"));
				else {
					if (cp.getDomainInfo().getScope() == null
							|| !cp.getDomainInfo().getScope().equals(domainInfo.getScope()))
						result.getErrors().add(new IgamtObjectError("Link_Scope_notMatched", cp.getId(),
								Type.CONFORMANCEPROFILE, new CPMetadata(cp),
								"In conformance profile Repository, " + cp.getLabel()
										+ "'s scope value of the Link is not matched with actual conformance profile scope.",
								null, "ERROR", "Internal"));
					if (cp.getDomainInfo().getVersion() != null
							&& !cp.getDomainInfo().getVersion().equals(domainInfo.getVersion()))
						result.getErrors().add(new IgamtObjectError("Link_Version_notMatched", cp.getId(),
								Type.CONFORMANCEPROFILE, new CPMetadata(cp),
								"In conformance profile Repository, " + cp.getLabel()
										+ "'s version value of the Link is not matched with actual conformance profile version.",
								null, "ERROR", "Internal"));
