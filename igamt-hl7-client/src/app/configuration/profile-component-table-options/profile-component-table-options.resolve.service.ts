...
@Injectable()
export class ProfileComponentTableOptionsResolve implements Resolve<any> {
  constructor(private tableOptionService: TableOptionsService, private router: Router) { }

  resolve(route: ActivatedRouteSnapshot): Promise<any> | boolean {
    return this.tableOptionService.getProfileComponentTableOptions().then(tableOptions => {
      return tableOptions;
    }).catch(error => {
